...
class Master extends EventEmitter
{
  ...
  /**
   * @param {number} readStartingAddress
   * @param {number} readQuantity
   * @param {number} writeStartingAddress
   * @param {Buffer} writeValues
   * @param {(Object|function(?Error, ?ReadWriteMultipleRegistersResponse))} [options]
   * @returns {Transaction}
   ...
   * is already managed by this `Master`.
   */
  readWriteMultipleRegisters(readStartingAddress, readQuantity, writeStartingAddress, writeValues, options)
  {
    return this.request(
      new messages.ReadWriteMultipleRegistersRequest(
        readStartingAddress,
        readQuantity,
  ...
  startRunningTransaction(runningTransaction)
  {
    ...
    setImmediate(runningTransaction.start.bind(runningTransaction, requestId));
  }
  ...
  handleTransactionError(runningTransaction, error)
  {
    ...
    {
      this.scheduleReExecution(runningTransaction);
    }
   ...
   * @param {RunningTransaction} runningTransaction
   */
  scheduleReExecution(runningTransaction)
  {
    this.cacheRunningTransaction(runningTransaction);
    this.transactionQueue.unshift(runningTransaction.transaction);
  }
 ...
 * @property {boolean} [defaultUnit=0]
