...
var codeToMessageMap = {
  0x01: 'Illegal Function Code: The function code received in the query is not an allowable action for the server (or slave).',
  0x02: 'Illegal Data Address: The data address received in the query is not an allowable address for the server (or slave).',
  0x03: 'Illegal Data Value: A value contained in the query data field is not an allowable value for server (or slave).',
  ...
  0x05: 'Acknowledge: The server (or slave) has accepted the request and is processing it, but a long duration of time will be required to do so.',
  0x06: 'Slave Device Busy: The server (or slave) is engaged in processing a long–duration program command.',
  0x07: 'Negative Acknowledge: The server (or slave) cannot perform the program function received in the query.',
  0x08: 'Memory Parity Error: The server (or slave) attempted to read record file, but detected a parity error in the memory.',
  0x0A: 'Gateway Path Unavailable: Gateway was unable to allocate an internal communication path from the input port to the output port for processing the request.',
  0x0B: 'Gateway Target Device Failed To Respond: No response was obtained from the target device.'
};
 ...
 * @constructor
 * @extends {h5.modbusfunctions.Response}
 * @param {number} functionCode A code of the function that resulted in the exception.
 ...
 * @return {h5.modbus.functions.ExceptionResponse} A response created from the specified `options`.
 */
 ...
 * @return {h5.modbus.functions.ExceptionResponse} A response created from its binary representation.
...
/**
 * @return {number} A code of the exception.
 */
