 ...
 *
 * @name h5.modbus.functions.ReadHoldingRegistersResponse
 * @constructor
 * @extends {h5.modbus.functions.Response}
 * @param {Buffer} values Values of the registers. A buffer of even length between 2 and 250.
 * @throws {Error} If the length of the `values` buffer is not between 1 and 250.
 */
function ReadHoldingRegistersResponse(values)
{
  ...
  if (values.length % 2 !== 0 || values.length < 1 || values.length > 250)
  {
    throw new Error(util.format(
      "The length of the `values` buffer must be an even number between 1 and 250, got: %d",
      values.length
    ));
  }
 ...
 *     Values of the registers. Must be a buffer of even length between 2 and 250.
 *
 * @param {object} options An options object.
 * @param {Buffer} options.values
 * @return {h5.modbus.functions.ReadHoldingRegistersResponse} A response created from the specified `options`.
 ...
 * @return {h5.modbus.functions.ReadHoldingRegistersResponse} A response created from its binary representation.
 * @throws {Error} If the specified buffer is not a valid binary representation of the read holding registers response.
 */
...
/**
 * @return {number} A number of the register values.
 */
