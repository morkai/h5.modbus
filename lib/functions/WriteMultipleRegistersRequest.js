...
var WriteMultipleRegistersResponse = require('./WriteMultipleRegistersResponse');
 ...
 *
 * @name h5.modbus.functions.WriteMultipleRegistersRequest
 * @constructor
 * @extends {h5.modbus.functions.Request}
 * @param {number} address A starting address. A number between 0 and 0xFFFF.
 * @param {Buffer} values Values of the registers. A buffer of even length between 2 and 256.
 * @throws {Error} If the `address` is not a number between 0 and 0xFFFF.
 * @throws {Error} If the `values` is not a Buffer of even length between 2 and 256.
 */
function WriteMultipleRegistersRequest(address, values)
{
  Request.call(this, 0x10);

  if (values.length % 2 !== 0 || values.length < 2 || values.length > 256)
  {
    throw new Error(util.format(
      "The length of the `values` Buffer must be an even number between 2 and 256, got: %d",
      values.length
    ));
  }
 ...
 *     Values of the registers. Must be a buffer of even length between 2 and 256.
 *
 * @param {object} options An options object.
 * @param {number=} options.address
 * @param {Buffer} options.values
 * @return {h5.modbus.functions.WriteMultipleRegistersRequest} A response created from the specified `options`.
 ...
 * @return {h5.modbus.functions.WriteMultipleRegistersRequest} A request created from its binary representation.
 ...
 * @throws {Error}
 */
WriteMultipleRegistersRequest.prototype.createResponse = function(responseBuffer)
{
...
/**
 * @return {Buffer} Values of the registers
 */
