...
{
  const bitArray = new Array(bitCount);
  const byteCount = byteArray.length;
  let bitArrayLength = 0;

  for (; byteIndex < byteCount; ++byteIndex)
  {
    const byteValue = byteArray[byteIndex];
...
ModbusHelpers.prepareRegisterValue = function(registerValue)
{
  if (Buffer.isBuffer(registerValue))
  {
    ...
    return registerValue.readUInt16BE(0);
  }
 ...
 */
ModbusHelpers.prepareRegisterValues = function(registerValues, maxBufferLength)
{
  if (!maxBufferLength)
  {
    maxBufferLength = 246;
  ...
  {
    buffer = Buffer.allocUnsafe(registerValues.length * 2);
    ...
    for (let i = 0; i < registerValues.length; ++i)
    {
      ...
      buffer.writeUInt16BE(n >= 0 ? n : (n + 0x10000), i * 2);
    }
  }
  ...
  if (buffer.length % 2 !== 0 || buffer.length < 2 || buffer.length > maxBufferLength)
  {
    throw new Error(
      'Invalid register values. '
      + `Expected a Buffer of even length between 2 and ${maxBufferLength} bytes, but got [${buffer.length}].`
    );
  }
