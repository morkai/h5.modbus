...
class UdpListener extends Listener
{
  ...
  open()
  {
    ...
    this.socket.bind({
      ...
      address: this.socketOptions.host,
  ...
  setUpClient(udpRemoteInfo)
  {
    ...
    newClient.on('write', this.onClientWrite.bind(this, key));
  ...
  onClientWrite(clientKey, data)
  {
    ...
    try
    {
      this.socket.send(data, 0, data.length, client.remoteInfo.port, client.remoteInfo.address);
    }
    catch (err)
    {
