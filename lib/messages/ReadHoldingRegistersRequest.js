...
class ReadHoldingRegistersRequest extends Request
{
   ...
   * @param {number} [options.quantity=1] A quantity of registers.
   * If specified, must be a number between 1 and 125. Defaults to 1.
   * @returns {ReadHoldingRegistersRequest}
   * @throws {Error} If any of the specified `options` are invalid.
   */
  static fromOptions(options)
  {
    return new ReadHoldingRegistersRequest(options.startingAddress, options.quantity);
  }
  ...
  static fromBuffer(buffer)
  {
    ...
    helpers.assertFunctionCode(buffer[0], FunctionCode.ReadHoldingRegisters);
  ...
  get endingIndex()
  {
    return this.startingIndex + this.quantity * 2;
  }
  ...
  toString()
  {
    return helpers.formatRequest(
      this,
      `Read ${this.quantity} holding registers starting from address ${this.startingAddress}`
    );
  }
}
