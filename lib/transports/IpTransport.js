 ...
 * @return {h5.modbus.errors.InvalidResponseDataError|null}
 */
IpTransport.Header.prototype.validate = function(transaction)
{
  ...
  {
    message = util.format(
      "Invalid version specified in the MODBUS response header. Expected: 0, got: %d",
      this.version
    );
  }
  else if (this.length === 0)
  {
    message = "Invalid length specified in the MODBUS response header. Expected: at least 1, got: 0.";
  }
  else if (this.unit !== expectedUnit)
  {
    message = util.format(
      "Invalid unit specified in the MODBUS response header. Expected: %d, got: %d.",
      expectedUnit,
      this.unit
    );
  }
