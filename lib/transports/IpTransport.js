...
class IpTransport extends Transport
{
  ...
  encode(id, unit, pdu)
  {
    const ipFrame = Buffer.allocUnsafe(2 + 2 + 2 + 1 + pdu.length);

    ipFrame.writeUInt16BE(id, 0);
    ipFrame.writeUInt16BE(0, 2);
    ipFrame.writeUInt16BE(pdu.length + 1, 4);
    ipFrame.writeUInt8(unit, 6);
    pdu.copy(ipFrame, 7);
  ...
  update(ipFrame, id)
  {
    if (ipFrame.length >= 2)
    {
      ipFrame.writeUInt16BE(id & 0xFFFF, 0);
